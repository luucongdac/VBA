...
class VALIDATE_TREATMENT_ESS_MENU(SETUP_VIEW_DISPLAY_STATUS_CHECK):
    name = "Confirm that MCR_Treatment_ESS_Menu (mcr_treatment_ess_menu.v) is displayed"
    view_name = "mcr_treatment_ess_menu"
    expected_result = "1"
...
class __M_PL_51__(ClinicalIntegrationTestProcedure):
    def __init__(self, test):
        name = 'Simulate & Validate "ESS image(A2)" in Treatment session" - Show view MCR_Service_ESS_Layout & Show menu MCR_Treatment_ESS_Menu on left menu panel'
        info_exchange = [
                        ]
