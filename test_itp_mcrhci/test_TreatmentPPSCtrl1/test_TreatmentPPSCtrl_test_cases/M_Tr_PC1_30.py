...
class SET_VARIABLE_MONITORING_POSITION_DEG_PITCH(SETUP_SET_VARIABLE):
    var_name = "tr1_pcu_pps_pitch_degmonitoring"
    value    = 10.0

class SET_VARIABLE_FOLLOWING_ERROR_DEG_PITCH(SETUP_SET_VARIABLE):
    var_name = "tr1_pcu_pps_pitch_followingerror"
    value    = 10.0

class VALIDATE_FEEDBACK_POSITION_DEG_PITCH(SETUP_VALIDATE_OBJECT_INFO):
    object_name = "mcr_treatment_pps_ctrl1:pcu4_pitchdegfbk"
    expected_value = 10.0
    info_type = "TMMI_MCR_OBJECT_GET_VALUE"

class VALIDATE_MONITORING_POSITION_DEG_PITCH(SETUP_VALIDATE_OBJECT_INFO):
    object_name = "mcr_treatment_pps_ctrl1:pcu4_pitchdegmonitoringfbk"
    expected_value = 10.0
    info_type = "TMMI_MCR_OBJECT_GET_VALUE"

class VALIDATE_FOLLOWING_ERROR_DEG_PITCH(SETUP_VALIDATE_OBJECT_INFO):
    object_name = "mcr_treatment_pps_ctrl1:pcu4_pitchfollowingerrfbk"
    expected_value = 10.0
    info_type = "TMMI_MCR_OBJECT_GET_VALUE"

class __M_Tr_PC1_30__(ClinicalIntegrationTestProcedure):
    def __init__(self, test):
        name = "Validate value of label Feedback Position, Monitoring Position and Following Error of Pitch in Treatment PPS Ctrl1 screen"
        info_exchange = [
                        ]
