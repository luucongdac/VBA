...
class SET_VARIABLE_MONITORING_POSITION_DEG_ROLL(SETUP_SET_VARIABLE):
    var_name = "tr2_pcu_pps_roll_degmonitoring"
    value    = 10.0

class SET_VARIABLE_FOLLOWING_ERROR_DEG_ROLL(SETUP_SET_VARIABLE):
    var_name = "tr2_pcu_pps_roll_followingerror"
    value    = 10.0

class VALIDATE_FEEDBACK_POSITION_DEG_ROLL(SETUP_VALIDATE_OBJECT_INFO):
    object_name = "mcr_treatment_pps_ctrl2:pcu4_rolldegfbk"
    expected_value = 10.0
    info_type = "TMMI_MCR_OBJECT_GET_VALUE"

class VALIDATE_MONITORING_POSITION_DEG_ROLL(SETUP_VALIDATE_OBJECT_INFO):
    object_name = "mcr_treatment_pps_ctrl2:pcu4_rolldegmonitoringfbk"
    expected_value = 10.0
    info_type = "TMMI_MCR_OBJECT_GET_VALUE"

class VALIDATE_FOLLOWING_ERROR_DEG_ROLL(SETUP_VALIDATE_OBJECT_INFO):
    object_name = "mcr_treatment_pps_ctrl2:pcu4_rollfollowingerrfbk"
    expected_value = 10.0
    info_type = "TMMI_MCR_OBJECT_GET_VALUE"

class __M_Tr_PC2_31__(ClinicalIntegrationTestProcedure):
    def __init__(self, test):
        name = "Validate value of label Feedback Position, Monitoring Position and Following Error of Roll in Treatment PPS Ctrl2 screen"
        info_exchange = [
                        ]
