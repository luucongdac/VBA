...
class THRIVER_WAIT_20_SECONDS(SETUP_THRIVER_WAIT):
    time = 20
class THRIVER_WAIT_15_SECONDS(SETUP_THRIVER_WAIT):
    time = 15

class DELETE_EQUIPMENT_PARAMETERS_FILE(SETUP_DELETE_FILE):
    name = "Delete equipment_parameters.txt file in ./hp/log/system/, and check that the file no longer exists"
...
class CHECK_CONTENT_EQUIPMENT_PARAMETERS_FILE(SETUP_CHECK_CONTENT_FILE):
    name = "Confirm that equiment_parameter.txt is created successfully in ./hp/log/system/"
    path_check=''.join([os.environ['TCS_ROOT'], '/runtime/hp/log/system/equipment_parameters.txt'])
    content_file = \
...
Testing procedure:
 (1) Delete equipment_parameters.txt file in ./hp/log/system/
 (2) Click on Save Eqt Params button
* Confirm after step (2)
  + Confirm equiment_parameter.txt is created successfully in ./hp/log/system/    
'''    

class EXECUTE_CMD_LINUX_ADD_FULL_PERMISION_FOR_THE_LOG_FILE(SETUP_EXECUTE_CMD_LINUX):
    name = "Change permisson of file equipment_parameters.txt to 777"
    cmd = "chmod 777 $TCS_ROOT/runtime/hp/log/system/equipment_parameters.txt ;\
            ls -la $TCS_ROOT/runtime/hp/log/system/ | grep equipment_parameters.txt"

class __M_PL_11__(ClinicalIntegrationTestProcedure):
    def __init__(self, test):
        name = 'Simulate & Validate "Save Eqt Params Button(B12)", Save equipment_parameters.txt file in ./hp/log/system/'
        info_exchange = [
                        ]
        ClinicalIntegrationTestProcedure.__init__(self, test, name=name, info_exchange=info_exchange)
        self.applicable_rooms = ['mcr']
        self.setup =    [
                        THRIVER_WAIT_15_SECONDS,
                        ...
                        CHECK_CONTENT_EQUIPMENT_PARAMETERS_FILE,
                        ]
        self.teardown_path = []
