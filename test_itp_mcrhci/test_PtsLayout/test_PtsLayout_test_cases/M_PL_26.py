...
class DELETE_NOZZLE2_PARAMETERS_FILE(SETUP_DELETE_FILE):
    name = "Delete nozzle2_parameters.txt file in ./hp/log/system/, and check that file no longer exists"
...
class CHECK_CONTENT_NOZZLE2_PARAMETERS_FILE(SETUP_CHECK_CONTENT_FILE):
    name = "Confirm that nozzle2_parameters.txt is created successfully in ./hp/log/system/"
    path_check = ''.join([os.environ['TCS_ROOT'], '/runtime/hp/log/system/nozzle2_parameters.txt'])
    content_file = \
...
Testing procedure:
 (1) Delete nozzle2_parameters.txt file in ./hp/log/system/
 (2) Click on Save TR2 Nozzle button
* Confirm after step (2)
  + Confirm nozzle2_parameters.txt is created successfully in ./hp/log/system/
'''    

class EXECUTE_CMD_LINUX_ADD_FULL_PERMISION_FOR_THE_LOG_FILE(SETUP_EXECUTE_CMD_LINUX):
    name = "Change permisson of file nozzle2_parameters.txt to 777"
    cmd = "chmod 777 $TCS_ROOT/runtime/hp/log/system/nozzle2_parameters.txt ;\
            ls -la $TCS_ROOT/runtime/hp/log/system/ | grep nozzle2_parameters.txt"

class __M_PL_26__(ClinicalIntegrationTestProcedure):
    def __init__(self, test):
        name = 'Simulate & Validate "Save TR2 Nozzle Button(B14), Save nozzle2_parameters.txt file in ./hp/log/system/'
        info_exchange = [
                        ]
        ...
        self.setup =    [
                        ...
                        DELETE_NOZZLE2_PARAMETERS_FILE,
                        THRIVER_WAIT_1_SECONDS,
