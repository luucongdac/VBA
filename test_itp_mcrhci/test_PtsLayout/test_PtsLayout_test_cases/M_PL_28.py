...
class CHECK_EMPTY_CONTENT_NOZZLE2_PARAMETERS_FILE(SETUP_CHECK_CONTENT_FILE):
    name = "Check empty content of file nozzle2_parameters.txt"
    path_check = ''.join([os.environ['TCS_ROOT'], '/runtime/hp/log/system/nozzle2_parameters.txt'])

    content_file = ""

class CHANGE_PERMISSION_000_TO_NOZZLE2_PARAMETERS_FILE_AND_RECHECK(SETUP_CHANGE_PERMISSION_AND_RECHECK_FILE):
    name = "Remove all permissons of file nozzle2_parameters.txt to 000"
    path_check = ''.join([os.environ['TCS_ROOT'], '/runtime/hp/log/system/nozzle2_parameters.txt'])
    permission_check = "000"

class CHANGE_PERMISSION_777_TO_NOZZLE2_PARAMETERS_FILE_AND_RECHECK(SETUP_CHANGE_PERMISSION_AND_RECHECK_FILE):
    name = "Restore all permissions of file nozzle2_parameters.txt to 777"
    path_check = ''.join([os.environ['TCS_ROOT'], '/runtime/hp/log/system/nozzle2_parameters.txt'])
...
class VALIDATE_CONTENT_POP_UP(SETUP_VALIDATE_POP_UP_CONTENT):
    content_popup = "Failed to open nozzle2_parameters.txt for writing"
...
Testing procedure:
 ...
 (2) Remove R/W/X permission fo the file nozzle2_parameters.txt
 (3) Click on Save TR2 Nozzle param button
* Confirm after step (3)
  + Confirm warining message pop up "Failed to open nozzle2_parameters.txt for writing"
'''    
    
class __M_PL_28__(ClinicalIntegrationTestProcedure):
    def __init__(self, test):
        name = 'Simulate & Validate "Save TR2 Nozzle Button(B14), nozzle2_parameters.txt file is busy in /hp/log/system/ for editing  '
        info_exchange = [
                        ]
        ...
        self.setup =    [
                        ...
                        THRIVER_WAIT_1_SECONDS,
                        CHANGE_PERMISSION_000_TO_NOZZLE2_PARAMETERS_FILE_AND_RECHECK,
                        THRIVER_WAIT_1_SECONDS,
                        CLICK_SAVE_TR2_NOZZLE_BUTTON,
                        THRIVER_WAIT_5_SECONDS,
                        VALIDATE_CONTENT_POP_UP,
                        CLICK_OK_POP_UP,
                        CHANGE_PERMISSION_777_TO_NOZZLE2_PARAMETERS_FILE_AND_RECHECK,
                        ]
        self.teardown_path = []
