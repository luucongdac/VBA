...
class SET_VARIABLE_RAW_TORQUE_Y(SETUP_SET_VARIABLE):
    var_name = "tr2_pcu_loadcell_torquez_raw"
    value    = 10.0

class SET_VARIABLE_RAW_OFFSET_TORQUE_Y(SETUP_SET_VARIABLE):
    var_name = "tr2_pcu_loadcell_torquez_raw_offset"
    value    = 10.0
...
class VALIDATE_RAW_FORCE_Y(SETUP_VALIDATE_OBJECT_INFO):
    object_name = "mcr_treatment_pps_ctrl2:pcu4_yforceraw"
    expected_value = "N/A"
    info_type = "TMMI_MCR_TEXT_FIELD"

class VALIDATE_RAW_OFFSET_FORCE_Y(SETUP_VALIDATE_OBJECT_INFO):
    object_name = "mcr_treatment_pps_ctrl2:pcu4_yforcerawoffset"
    expected_value = "N/A"
    info_type = "TMMI_MCR_TEXT_FIELD"

class VALIDATE_ENG_FORCE_Y(SETUP_VALIDATE_OBJECT_INFO):
    object_name = "mcr_treatment_pps_ctrl2:pcu4_yforceeng"
    expected_value = "N/A"
    info_type = "TMMI_MCR_TEXT_FIELD"

class VALIDATE_RAW_TORQUE_Y(SETUP_VALIDATE_OBJECT_INFO):
    object_name = "mcr_treatment_pps_ctrl2:pcu4_ytorqueraw"
    expected_value = 10.0
    info_type = "TMMI_MCR_OBJECT_GET_VALUE"

class VALIDATE_RAW_OFFSET_TORQUE_Y(SETUP_VALIDATE_OBJECT_INFO):
    object_name = "mcr_treatment_pps_ctrl2:pcu4_ytorquerawoffset"
    expected_value = 10.0
    info_type = "TMMI_MCR_OBJECT_GET_VALUE"

class VALIDATE_ENG_TORQUE_Y(SETUP_VALIDATE_OBJECT_INFO):
    object_name = "mcr_treatment_pps_ctrl2:pcu4_ytorqueeng"
    expected_value = 10.0
    info_type = "TMMI_MCR_OBJECT_GET_VALUE"

class __M_Tr_PC2_33__(ClinicalIntegrationTestProcedure):
    def __init__(self, test):
        name = "Validate value of label Force and Torque of Y in Treatment PPS Ctrl2 screen"
        info_exchange = [
                        ]
