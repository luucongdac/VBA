...
class CHECK_EMPTY_CONTENT_NOZZLE1_PARAMETERS_FILE(SETUP_CHECK_CONTENT_FILE):
    name = "Confirm that nozzle1_parameters.txt is existed with empty content"
...
class CHECK_CONTENT_NOZZLE1_PARAMETERS_FILE(SETUP_CHECK_CONTENT_FILE):
    name = "Confirm that the existing nozzle1_parameters.txt in ./hp/log/system/ is updated"
    path_check = ''.join([os.environ['TCS_ROOT'], '/runtime/hp/log/system/nozzle1_parameters.txt'])
    content_file = \
...
nozzle1_parameters.txt already exist.

Testing procedure:
 (1) Navigate to PTS_Layout screen
 (2) Click on Save TR1 Nozzle button
* Confirm after step (2)
  + Confirm existing nozzle1_parameters.txt in ./hp/log/system/ is updated.
'''    
    
class __M_PL_21__(ClinicalIntegrationTestProcedure):
    def __init__(self, test):
        name = 'Simulate & Validate "Save TR1 Nozzle Button(B13), Update existing nozzle1_parameters.txt file in ./hp/log/system/ '
        info_exchange = [
                        ]
