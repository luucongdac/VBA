...
class CHECK_EMPTY_CONTENT_EQUIPMENT_PARAMETERS_FILE(SETUP_CHECK_CONTENT_FILE):
    name = "Confirm that equiment_parameter.txt is existed with empty content"
...
class CHECK_CONTENT_EQUIPMENT_PARAMETERS_FILE(SETUP_CHECK_CONTENT_FILE):
    name = "Confirm that the existing equiment_parameter.txt in ./hp/log/system/ is updated"
    path_check = ''.join([os.environ['TCS_ROOT'], '/runtime/hp/log/system/equipment_parameters.txt'])
    content_file = \
...
* Confirm after step (2)
  + Confirm existing equiment_parameter.txt in ./hp/log/system/ is updated.
'''    
    
class __M_PL_12__(ClinicalIntegrationTestProcedure):
    def __init__(self, test):
        name = 'Simulate & Validate "Save Eqt Params Button(B12)", Update existing equipment_parameters.txt file in ./hp/log/system/'
        info_exchange = [
                        ]
