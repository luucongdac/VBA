...
class SET_VARIABLE_BLIP_COLLISION_DETECTED_OK(SETUP_SET_VARIABLE):
    var_name = "tr2_pcu_loadcell_collision_detected"
    value    = True

class VALIDATE_BLIP_COLLISION_DETECTED_OK(SETUP_VALIDATE_OBJECT_INFO):
    object_name = "mcr_treatment_pps_ctrl2:pcu4_coldetected"
...
class __M_Tr_PC2_35__(ClinicalIntegrationTestProcedure):
    def __init__(self, test):
        name = "Validate blip of Collision Detected in Treatment PPS Ctrl2 screen"
        info_exchange = [
                        ]
