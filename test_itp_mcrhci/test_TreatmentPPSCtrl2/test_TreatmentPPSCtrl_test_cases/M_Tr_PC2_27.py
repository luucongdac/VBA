...
class SET_VARIABLE_MONITORING_POSITION_CM_Y(SETUP_SET_VARIABLE):
    var_name = "tr2_pcu_pps_longitudinal_cmmonitoring"
    value    = 10.0

class SET_VARIABLE_FOLLOWING_ERROR_CM_Y(SETUP_SET_VARIABLE):
    var_name = "tr2_pcu_pps_longitudinal_followingerror"
    value    = 10.0

class VALIDATE_FEEDBACK_POSITION_CM_Y(SETUP_VALIDATE_OBJECT_INFO):
    object_name = "mcr_treatment_pps_ctrl2:pcu4_ycmfbk"
    expected_value = 10.0
    info_type = "TMMI_MCR_OBJECT_GET_VALUE"

class VALIDATE_MONITORING_POSITION_CM_Y(SETUP_VALIDATE_OBJECT_INFO):
    object_name = "mcr_treatment_pps_ctrl2:pcu4_ycmmonitoringfbk"
    expected_value = 10.0
    info_type = "TMMI_MCR_OBJECT_GET_VALUE"

class VALIDATE_FOLLOWING_ERROR_CM_Y(SETUP_VALIDATE_OBJECT_INFO):
    object_name = "mcr_treatment_pps_ctrl2:pcu4_yfollowingerrfbk"
    expected_value = 10.0
    info_type = "TMMI_MCR_OBJECT_GET_VALUE"

class __M_Tr_PC2_27__(ClinicalIntegrationTestProcedure):
    def __init__(self, test):
        name = "Validate value of label Feedback Position, Monitoring Position and Following Error of Y in Treatment PPS Ctrl2 screen"
        info_exchange = [
                        ]
